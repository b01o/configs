...
local o = {
    ...
    ext = "mkv",
    command_template = [[
        ffmpeg -hwaccel auto -i "$in" -ss $shift -t $duration -c:v $vcodec -c:a $acodec $audio -map 0 -sn $opts "$out.$ext"
    ]],
}
...
function get_homedir()
  ...
  return os.getenv("MPV_CHOPCHOP_OUTPUT_DIR") or os.getenv("HOME") or os.getenv("USERPROFILE") or ""
end
...
function get_outname(shift, endpos)
    ...
    name = name:gsub(" ", "_")
    name = name:gsub(":", "-")
    name = name .. string.format(".part[%s-%s]", timestamp(shift), timestamp(endpos))
    return name
end
